        ...
        export class SOMMap extends D3ChartObject {
            ...
            public ShowVisMap(visData: MapData, classifierID: string): void {
                ...
                svg.append('text')
                    ...
                    .text(() => new Date(+visData.mapID.substr(0, 4), +visData.mapID.substr(4, 2), +visData.mapID.substr(6, 2), +visData.mapID.substr(8, 2), +visData.mapID.substr(10, 2), +visData.mapID.substr(12, 2)).toLocaleString());
                svg.append('text')
                    .attr('y', this._top_offset + 16 * this._unit_height)
                    .attr('x', this._left_offset + 16 * this._unit_width)
                    .attr('text-anchor', 'middle')
                    .attr('alignment-baseline', 'hanging')
                    .text(visData.unitsData.reduce((sum, d) => sum + d.value, 0) + ' tweets');
                /*=============Map metaInfo===================*/
                ...
                var fontSizeScale = d3.scale.pow().domain(d3.extent(visData.labels, d => d.value )).range([10, 30]);
