...
(function (Chart) {
    ...
    var Bar = (function (_super) {
        ...
        Bar.prototype.render = function (data) {
            ...
            var maxY = d3.max(stackData, function (d) {
            ...
            var barGroups = svg.selectAll('g.bars').data(stackData).enter().append('g').attr('class', 'bars').style('fill', function (d, i) {
            ...
            // Legend items
            var legendItem = legendGroup.selectAll('g.legendItem').data(stackData).enter().append('g').attr('class', 'legendItem').style('fill', function (d, i) {
                return _this.colors[i];
            }).attr('transform', function (d, i) {
                return 'translate(0, ' + (_this.legendItemHeight * (2 - i)) + ')';
            });
            ...
            legendItem.append('text').text(function (d) {
                return d.desc;
            }).attr('x', 30).attr('dy', '1em');

            // Bars
            var rects = barGroups.selectAll('rect').data(function (d) {
                return d.data;
            }).enter().append('rect').attr('x', function (d, i) {
                return x(d.x);
            }).attr('y', function (d, i) {
                return _this.chartHeight - y(d.y + d.y0);
            ...
            rects.append('title').text(function (d) {
                return _this.iso8601(d.x) + ' - ' + d.y + 'ms';
            });
    ...
    var DailyBuild = (function (_super) {
        ...
        DailyBuild.prototype.render = function (data) {
            ...
            var rects = g.selectAll('rect').data(function (d, i) {
                ...
                return 'day ' + _this.getTextDataString(d);
            }).attr('x', function (d, i) {
                return _this.labelWidth + _this.labelGutter + x(d.date);
            }).attr('width', boxSize).attr('height', boxSize);

            rects.append('title').text(function (d) {
                return _this.iso8601(d.date) + ' - ' + _this.getTextDataString(d);
            });

            var ticks = x.ticks(d3.time.mondays, 1);

            // Date text boxes
            svg.append('g').attr('class', 'dates').selectAll('text').data(ticks).enter().append('text').text(function (d) {
                return _this.iso8601(d);
            }).attr('transform', function (d, i) {
                return 'translate(' + (_this.labelWidth + _this.labelGutter + x(d) + 5) + ', ' + y(data.length + 1) + ')';
            }).attr('text-anchor', 'start');

            // Vertical hashes at week boundaries
            svg.append('g').attr('class', 'hashes').selectAll('line').data(ticks).enter().append('line').attr('x1', function (d) {
                return _this.labelWidth + _this.labelGutter + x(d);
            }).attr('x2', function (d) {
                return _this.labelWidth + _this.labelGutter + x(d);
            }).attr('y1', 0).attr('y2', y(data.length + 1));
...
var buildData = days.map(function (day) {
    return ({ date: day, pass: Math.random() > 0.1 });
});
var compilerTestData = days.map(function (day) {
    return ({ date: day, pass: Math.random() > 0.1 });
});
var servicesTestData = days.map(function (day) {
    return ({ date: day, pass: Math.random() > 0.1 });
});
...
var parseData = days.map(function (day) {
    return ({ x: day, y: parseRandom() });
});
var typecheckData = days.map(function (day) {
    return ({ x: day, y: typecheckRandom() });
});
var emitData = days.map(function (day) {
    return ({ x: day, y: emitRandom() });
});
